...
from ._utils import _set_device, validate_and_set_device, timeit_wrapper
...
class CTGAN(BaseSynthesizer):
    ...
    @random_state
    def fit(self, train_data, discrete_columns=(), epochs=None, save_dir=None):
        ...
        np.save('ctgan_train.npy', train_data)
        ...
        wandb.init(project='CTGAN')
        wandb.config = {
        "gen_lr": self._generator_lr,
        "dis_lr": self._discriminator_lr,
        "epochs": epochs,
        ...
        for i in epoch_iterator:
            for id_ in range(steps_per_epoch):
                ...
                loss_g.backward()
                optimizerG.step()
                epoch_loss_g += loss_g.detach().cpu().item()
                epoch_loss_d += loss_d.detach().cpu().item()

            generator_loss = loss_g.detach().cpu().item()
            discriminator_loss = loss_d.detach().cpu().item()
            epoch_lossg = epoch_loss_g / steps_per_epoch
            epoch_lossd = epoch_loss_d / steps_per_epoch
            ...
            if epoch_lossg < best_gen_loss:
                best_gen_loss = epoch_lossg
            ...
            wandb.log({"loss_gen":epoch_lossg, "loss_discriminator":epoch_lossd}, step=i)
