...
class MLPSynthesizer(nn.Module):
    ...
    def __init__(
        ...
        # condition embed label
        if isinstance(self.n_classes, list):  #condition이 여러컬럼
            self.condition_embs = nn.ModuleList([
                nn.Embedding(n_class, dim_t) for n_class in self.n_classes
            ])
            self.use_multi_label = True
            self.condition_projtype = 'mlp'  #'linear' or 'mlp'
        elif self.n_classes is not None:  #condition이 단일 컬럼 
            self.label_emb = nn.Embedding(self.n_classes, dim_t)
            self.use_multi_label = False
        else:
            self.label_emb = None
            self.use_multi_label = False
            
        #multi condition linear layer
        if self.use_multi_label:
            concat_dim = len(self.n_classes) * dim_t
            if self.condition_projtype == 'linear':
                self.condition_proj = nn.Linear(concat_dim, dim_t)
            elif self.condition_projtype == 'mlp':
                self.condition_proj = nn.Sequential(
                    nn.Linear(concat_dim, dim_t*3),
                    nn.SiLU(),
                    nn.Linear(dim_t*3, dim_t)
                )
    ...
    def forward(self, x, timesteps, label=None):
        ...
        if label is not None:
            if self.use_multi_label:
                # label: (B, n_label_cols)
                label_embs = [self.condition_embs[i](label[:, i]) for i in range(label.shape[1])]
                #label_emb = torch.stack(label_embs, dim=0).sum(dim=0)  
                label_emb = torch.cat(label_embs, dim=-1)
                label_emb = self.condition_proj(label_emb)
            else:
